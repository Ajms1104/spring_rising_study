...
public class OrderService implements IOrderRepositoryPort {
    ...
    private final IPurchaseOrderUseCase eventPublisher;
    ...
    public Order process(Long customerId, ShippingInfo shippingInfo, PaymentMethod paymentMethod, Coupon coupon){
        ...
        List<OrderItem> orderItems = cart.getItmes().stream()
        ...
        if (paymentSuccess){
            ...
            saveOrder.getDomainEvents().forEach(eventPublisher::publish);
            savedOrder.clearDomainEvents();
        }

        cartRepository.findByCustomerId(customerId)
            .ifPresent(completedCart -> {
                completedCart.getItem().clear();
