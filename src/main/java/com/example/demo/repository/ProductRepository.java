//상품의 Repository(Service 반환, 도메인 비지니스)
...
public class ProductRepository implements IOrderRepositoryPort {
    ...
    @Override
    public List<Product> findAll() {return store.values().stream().toList();}

    @Override
    public List<Product> findByKeyword(String keyword) {
        return store.values().stream()
            .filter(product -> product.getName().toLowerCase().cotains(keyword.toLowerCase()))
