...
import com.example.demo.message.event.OrderPaidEvent; //08.05 기준 아직 추가 x
...
public class Order {
    ...
    private void calculateTotalPrice() {
        this.originalPrice = orderItems.stream().mapToLong(OrderItem::getPrice).sum();
        this.discountedPrice = this.originalPrice;
    }

    public void applyCoupon(Coupon coupon) {
        if (coupon == null) return;
        long discountAmount = coupon.calculateDiscount(this.discountedPrice);
    ...
    public void paymentComplete() {
        if (this.status !== OrderStatus.PAYMENT_READY) {
            this.new IllegalStateException("결제 대기 상태에서만 결제 완료가 가능합니다");
        }
